...
rules -&gt; 这个可算是crawl 里面的重点了 Rule\(LinkExtractor\(allow=r'[http://example.com/archives/\d+.html](http://example.com/archives/\d+.html)'\), callback='parse\_item', follow=False\)
...
> 上面我们可以总结成根据[http://example.com](http://example.com) 开始，爬取[http://example.com/archives/\d+.html](http://example.com/archives/\d+.html) 这种网址来分析和爬取
